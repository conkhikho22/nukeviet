...
if (!nv_function_exists('nv_block_news_center')) {
    ...
    function nv_block_config_news_center($module, $data_block, $lang_block)
    {
        ...
        if ($nv_Request->isset_request('loadajaxdata', 'get')) {
            ...
            $html .= '	<div class="col-sm-18"><input type="text" name="config_numrow" class="form-control w100 pull-left" value="' . $data_block['numrow'] . '"/>';
            $html .= '	<span class="text-middle pull-left">&nbsp; ' . $lang_block['numshow'] . '&nbsp; </span>';
            $html .= '  <input type="width" name="config_numshow" class="form-control w100 pull-left" value="' . $data_block['numshow'] . '"/>';
            $html .= '  </div>';
            $html .= '</div>';

            $html .= '<div class="form-group">';
            $html .= '	<label class="control-label col-sm-6">' . $lang_block['img_size'] . ':</label>';
            ...
            $html .= '	<div class="col-sm-18">';
            $html .= '	<input type="text" class="form-control w100 pull-left" name="config_length_title" value="' . $data_block['length_title'] . '"/>';
            $html .= '	<span class="text-middle pull-left">&nbsp;' . $lang_block['length_hometext'] . '&nbsp;</span><input type="text" class="form-control w100 pull-left" name="config_length_hometext" value="' . $data_block['length_hometext'] . '"/>';
            $html .= '</div>';
            $html .= '</div>';
            ...
            $html .= '</select>';
            $html .= '	<span class="text-middle pull-left">&nbsp;' . $lang_block['tooltip_length'] . '&nbsp;</span><input type="text" class="form-control w100 pull-left" name="config_tooltip_length" value="' . $data_block['tooltip_length'] . '"/>';
            $html .= '</div>';
            $html .= '</div>';
    ...
    function nv_block_config_news_center_submit($module, $lang_block)
    {
        ...
        $return['config']['numshow'] = $nv_Request->get_int('config_numshow', 'post', 0);
    ...
    function nv_block_news_center($block_config)
    {
        ...
        if (isset($site_mods[$module])) {
            ...
            if (!empty($list)) {
                ...
                $xtpl = new XTemplate('global.block_news_center.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/blocks');
                ...
                foreach ($list as $row) {
                    ...
                    if ($block_config['showtooltip']) {
                        ...
                        $xtpl->parse('main.news_loop.tooltip');
                    }
                    
                    $xtpl->set_autoreset();
                    $xtpl->parse('main.news_loop');
                }
                $xtpl->assign('numshow', $block_config['numshow']);
                $xtpl->parse('main');
